...
class CenterNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nstack):
            ...
            hm_pred = _sigmoid(self.hm_out_conv[i](pred_features))
            wh_pred = self.wh_out_conv[i](pred_features)
            reg_pred = self.reg_out_conv[i](pred_features)

            outputs.append({'heatmap': hm_pred, 'wh': wh_pred, 'reg': reg_pred})
...
class MultiScaleCenterNet(nn.Module):
    def __init__(self, cfg, increase=0, eval=False):
        super(MultiScaleCenterNet, self).__init__()
        ...
        self.init_convs = nn.ModuleList([
            ...
            nn.Sequential(
                ConvBlock(3, 128, kernel_size=7, stride=1, padding=3, norm='batch', activation='relu'),
                ResidualUpBlock(128, 256, kernel_size=4, stride=2, padding=2),
            )
        ])
